...
namespace IIS.123456
{
    ...
    public class Контроллер : ICSSoft.STORMNET.DataObject
    {
        ...
        private IIS.123456.Сенсоры fСенсоры;
        
        private IIS.123456.Серверная fСерверная;
        ...
        [PropertyStorage(new string[] {
                "Сенсоры"})]
        [NotNull()]
        public virtual IIS.123456.Сенсоры Сенсоры
        {
            get
            {
                ...
                IIS.123456.Сенсоры result = this.fСенсоры;
        ...
        /// <summary>
        /// мастеровая ссылка на шапку IIS.123456.Серверная.
        /// </summary>
        ...
        [PropertyStorage(new string[] {
                "Серверная"})]
        public virtual IIS.123456.Серверная Серверная
        {
            get
            {
                ...
                IIS.123456.Серверная result = this.fСерверная;
        ...
        public class Views
        {
            ...
            public static ICSSoft.STORMNET.View КонтроллерE
            {
                get
                {
                    return ICSSoft.STORMNET.Information.GetView("КонтроллерE", typeof(IIS.123456.Контроллер));
                }
            }
        }
    }
    ...
    public class DetailArrayOfКонтроллер : ICSSoft.STORMNET.DetailArray
    {
        ...
        // *** Start programmer edit section *** (IIS.123456.DetailArrayOfКонтроллер members)

        // *** End programmer edit section *** (IIS.123456.DetailArrayOfКонтроллер members)
        ...
        /// </summary>
        public DetailArrayOfКонтроллер(IIS.123456.Серверная fСерверная) : 
                base(typeof(Контроллер), ((ICSSoft.STORMNET.DataObject)(fСерверная)))
        {
        }
        
        public IIS.123456.Контроллер this[int index]
        {
            get
            {
                return ((IIS.123456.Контроллер)(this.ItemByIndex(index)));
            }
        }
        
        public virtual void Add(IIS.123456.Контроллер dataobject)
        {
